...
parser = argparse.ArgumentParser(
    description='Append the date to your photos!.')
...
def get_minimum_creation_time(exif_data):
    ...
    elif 36868 in exif_data and exif_data[36868] < mtime:
...
def rename_picture2(fullFileName):
    ...
    # not ideal. modified times in windows are unreliable. Linux
        ...
        img.close()
        mod_date = time.strftime("%Y%m%d_%H%M%S", time.gmtime(
            os.path.getmtime(fullFileName)))
        try:
            rename_if_exists_move(fullFileName, filepath.with_name(
                date_taken + " - " + filename))
        except Exception as e:
...
def rename_if_exists_move(original_filepath, new_name):
    ...
    try:
        os.rename(original_filepath, new_name)
    except Exception as e:
        ...
        for name in files:
            ...
            try:
                foldername = time.strftime(
                    "%Y%m", time.strptime(date_from_name, "%Y%m%d_%H%M%S"))
                create_dir(os.path.join(rootdir, foldername))
                shutil.move(os.path.join(root, name),
                            os.path.join(rootdir, foldername, name))
            except Exception as e:
